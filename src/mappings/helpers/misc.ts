...
export function getKoilVaultSnapshot(vaultId: string, timestamp: i32): KoilVaultSnapshot {
  let dayID = timestamp / 86400;
  let id = vaultId + '-' + dayID.toString();
  let snapshot = KoilVaultSnapshot.load(id);

  if (snapshot == null) {
    let dayStartTimestamp = dayID * 86400;
    snapshot = new KoilVaultSnapshot(id);
