...
import { Pool, TokenPrice, KoilVault, PoolHistoricalLiquidity, LatestPrice } from '../types/schema';
...
import { createPoolSnapshot, getKoilVaultSnapshot, getToken, getTokenPriceId, loadPoolToken } from './helpers/misc';
...
export function updatePoolLiquidity(poolId: string, block: BigInt, pricingAsset: Address, timestamp: i32): boolean {
  ...
  // Update global stats
  let vault = KoilVault.load('2') as KoilVault;
  vault.totalLiquidity = vault.totalLiquidity.plus(liquidityChange);
  vault.save();

  let vaultSnapshot = getKoilVaultSnapshot(vault.id, timestamp);
