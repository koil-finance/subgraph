...
import { KoilVault, Pool, Swap, JoinExit, Investment, TokenPrice, UserInternalBalance } from '../types/schema';
import {
  tokenToDecimal,
  getTokenPriceId,
  scaleDown,
...
export function handleSwapEvent(event: SwapEvent): void {
  ...
  let vaultSnapshot = getKoilVaultSnapshot(vault.id, blockTimestamp);
